...
exports.getAllProducts = catchAsyncError(async (req, res, next) => {
  ...
  const apiFeature = new ApiFeatures(productModel, req.query)
    .search()
    .filter();
  // .pagination(resultPerPage);//or req.query.keyword .filter()
  // const products = await productModel.find();
  // const products = await apiFeature.query;
  let products = await apiFeature.query; //query is of apiFeature class
...
exports.updateProduct = catchAsyncError(async (req, res) => {
  ...
  if (!product) return next(new ErrorHandler("productModel not found", 404));
  ...
  res.status(200).json({
    success: true,
    product, //ie sending the updated product
  });
});
